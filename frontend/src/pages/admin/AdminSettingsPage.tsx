...
export function AdminSettingsPage() {
  ...
  // Everyone lounge-side reaches this page — it is where they turn on their own
  // notifications — but the booking-engine configuration is behind SETTINGS.
  const canSeeEngine = canView('SETTINGS');
  const { data, isPending, isError, error, refetch } = useBookingSettings();

  if (canSeeEngine && isPending) return <LoadingState />;
  if (canSeeEngine && isError) return <ErrorState error={error} onRetry={() => void refetch()} />;
            ...
            <section className="nu-panel">
              ...
              <div className="nu-panel__body">
                ...
                <p className="nu-hint" style={{ marginTop: 'var(--nu-space-4)' }}>
                  These are set through the server's environment variables. Working hours and
                  closures are edited in Availability.
                </p>
                {data.timezone === 'UTC' && (
                  <div
                    className="nu-notice nu-notice--warn"
